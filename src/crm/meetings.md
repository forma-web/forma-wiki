# Встречи

## 16.02.2023


### Присутствовали:

1.	Робонен А.С. 420-2
2.	Горяинов С.А. 420-3
3.	Васильчиков Д.А. 421-1
4.	Фёдоров Р.М. 421-1
5.	Туралин А.С. 421-1
6.	Деркачёв Е.И. 421-1
7.	Семёнов К.М. 421-1


### Обсуждалось:

1.	Установлен общий план на семестр:
  * Реализовать:
    * Базу лидов
    * Базу клиентов
    * Базу сделок
    * Отправку сообщений по контактам

2.	Определены сущности для базы данных:
    *	Лид
    *	Клиент
    *	Сделки
    *	Пользователь (Компания)
    *	Пользователь (Сотрудник)

3.	Определены наборы полей для сущностей:
  * Лид:
    *	Название
    *	Источник
    *	Статус
    *	Цель
    *	Дата
    *	Контакты
    *	Комментарий

  * Клиент:
    *	ФИО
    *	Пол
    *	Дата рождения
    *	Документы
    *	Контакты
    *	Сделки
    *	Платёжные данные
    *	Город
    *	Комментарий

  * Сделка:
    *	Статус
    *	Клиент(-ы)
    *	Услуга
    *	Документы
    *	Стоимость
    *	Платёжные данные
    *	Дата создания
    *	Дата завершения

  * Пользователь (Компания):
    *	Название
    *	Документы
    *	Контакты
    *	Сотрудники
    *	Логин (почта)
    *	Пароль

  * Пользователь (Сотрудник):
    *	ФИО
    *	Логин
    *	Пароль
    *	Компания 
    *	Должность
    *	Уровень доступа
    *	Филиал
    *	Контакты 


### Дальнейшие действия:

1.	Всем опробовать GitHub

2.	Фронтенд:
  *	Написать документацию
  *	Сделать макеты страниц
  *	Сделать подборки туров
  *	Поработать с формами 
  *	Сделать вёрстку макета

3.	Бэкенд:
  *	Рассмотреть несколько вариантов архитектуры проекта
  *	Изучить синтаксисы и документацию PHP, REST API, фреймворк Laravel
  *	Установить и настроить среду разработки  для WLS + Ubuntu, PHP 8.2, Composer, Docker Desktop, PHPStorm

4.	Документация:
  *	Составить отчёт по встрече
  *	Ознакомиться с Markdown синтаксисом
  *	Добавить и развернуть wiki
  *	Перенести отчёт на wiki

![Иерархия](https://user-images.githubusercontent.com/83452064/223128503-ea752f44-a067-4841-9635-e6842fd8e740.png)

***

## 02.03.2023


### Присутствовали:

1.	Робонен А.С. 420-2
2.	Васильчиков Д.А. 421-1
3.	Фёдоров Р.М. 421-1
4.	Туралин А.С. 421-1
5.	Деркачёв Е.И. 421-1
6.	Семёнов К.М. 421-1


### Обсуждалось:

1.	Проведённая работа за время с прошлой встречи:
  *	Уточнена аналитика
  *	Частично подготовлен дизайн
  *	Создана вики

2.	Написание научных статей:
  *	Плюсы:
    * Дополнительные баллы для магистратуры
    *	Повышенная стипендия
  *	Минусы:
    *	Большой объём работы

3.	Распределение ролей/разделение доступов:
  *	Подход гибкой настройки (как в Discord) – затруднителен.
    Выбран фиксированный функционал ролей.
  *	Возможности ролей:
    *	Админ (техническая помощь компании): Управление системой, управление руководителями
    *	Руководитель: Управление сотрудниками, отслеживание
    *	Сотрудник: Рабочий функционал 
  *	В перспективе можно будет добавить больше ролей сверху
 
4.	Обязательная защита личных данных клиентов:
  *	Варианты:
    *	Требования согласия высших должностей

5.	Необходимость раннего подробного планирования продукта во избежание лишнего переписывания кода.

6.	Первая защита:
  Не сильно напрягают на первой защите. Основные требования: что делали, что изучали, дальнейшие планы. Несильные требования к программе.

7.	Вектор реализации:
  *	Сверху-вниз (от ролей к функциям)
    *	Проблема: будет изначально пустышка с заглушками/плейсхолдерами 
  *	Снизу-вверх (от функций к ролям)


### Дальнейшие действия:

1.	Общее:
  *	Отслеживать проведённую работу и всё записывать для дальнейшего отчёта по семестру

2.	Фронтенд:
  *	Приготовить дизайн
  *	Помощь в аналитике: просмотреть функционал и дизайн других CRM-систем

3.	Бэкенд:
  *	 Продумать архитектуру

4.	Документация/аналитика:
  *	Подтянуть теорию
  *	Заполнить вики
  *	Продумать функционал
  *	Прописать функционал
  *	Подготовить ролевую модель
  *	Изучить юзер-кейсы
  *	Дополнительно: изучить и составить BPMN схемы

![Распределение ролей](https://user-images.githubusercontent.com/83452064/223133352-9d591b1e-d7af-4be8-80b9-49523cbe1484.png)

***

## 09.03.2023


### Присутствовали:

1.	Робонен А.С. 420-2
2.	Горяинов С.А. 420-3
3.	Туралин А.С. 421-1
4.	Деркачёв Е.И. 421-1
5.	Семёнов К.М. 421-1


### Обсуждалось:

1.	Проведённая работа:
  *	Перенесены отчёты на wiki
  *	Распределены задачи
  *	Составлена to-do для бэка
  *	Частично изучен синтаксис PHP

2.	Необходимость карточки проекта:
  *	Понимание сути работы
  *	Структуризация задач проекта 

3.	Заполнили карточку проекта

4.	Изучили конкурентов


### Дальнейшие действия:

1.	Фронтенд:
  *	Макет

2.	Бэкенд:
  *	Обучение

3.	Документация:
  *	Юзер-стори
  *	Юз-кейсы
  *	Словарь данных
  *	Функциональные требования
  *	Концепция системы оценивания
  *	Жизненный цикл оценивания сотрудника
  *	Изучить ранние наработки

***

## 16.03.2023


### Присутствовали:

1.	Робонен А.С. 420-2
2.	Горяинов С.А. 420-3
3.	Васильчиков Д.А. 421-1
4.	Фёдоров Р.М. 421-1
5.	Туралин А.С. 421-1
6.	Семёнов К.М. 421-1


### Обсуждалось:

1.	Проведённая работа за неделю:
  *	Макет дизайна
  *	Изучалась теория бэком
  *	Собран доп. материал для документа

2.	Время проведения встреч перенесено с 14:00 на 13:00

3.	Начало прописывания финального отчёта – конец Апреля.


### Дальнейшие действия:

1.	Фронтенд:
  *	Страница клиентов 

2.	Бэкенд:
  *	Скелет базы данных

3.	Документация:
  *	Изучение и заполнение документации
  *	Функционал

***

## 30.03.2023


### Присутствовали:

1.	Робонен А.С. 420-2
2.	Горяинов С.А. 420-3
3.	Фёдоров Р.М. 421-1
4.	Туралин А.С. 421-1
5.	Деркачёв Е.И. 421-1
6.	Семёнов К.М. 421-1


### Обсуждалось:

1.	Проведённая работа за неделю:
  *	Регистрация/вход/выход
  *	Рассмотрен качественный пример карточки
2.	План подготовки к итоговому отчёту и защите


### Дальнейшие действия:

1.	Общее
  *	Отмечать свои действия в доске задач

2.	Фронтенд:
  *	Сущности и их реализация в интерфейсе

3.	Бэкенд:
  *	Сущности и их реализация в базе данных

4.	Документация:
  *	Сущности (наивысший приоритет)
  *	Функционал интерфейса (приоритет)

***

## 02.04.2023


### Присутствовали:

1.	Робонен А.С. 420-2
2.	Фёдоров Р.М. 421-1
3.	Семёнов К.М. 421-1


### Обсуждалось:

1. Роли пользователя (уровни доступа) и их функции

![Роли и их функции](/Roles.png)

2. Иерархия ролей

![Иерархия ролей](/Hierarchy.png)

3. Процесс создания аккаунта и создания компании/присоединения к компании

![Регистрация](/Registration.png)

***

## 06.04.2023


### Присутствовали:

1.	Робонен А.С. 420-2
2.	Горяинов С.А. 420-3
3.	Фёдоров Р.М. 421-1
4.	Туралин А.С. 421-1
5.	Деркачёв Е.И. 421-1
6.	Семёнов К.М. 421-1


### Обсуждалось:

1.	Проведённая работа за неделю:
  *	Схемы с прошлой встречи: роли и их функции, иерархия и регистрация
  *	Начали верстать
  *	Доделали привязку позиций
  *	Сделали регистрацию

***

## 13.04.2023


### Присутствовали:

1.	Робонен А.С. 420-2
2.	Горяинов С.А. 420-3
3.	Васильчиков Д.А. 421-1
4.	Фёдоров Р.М. 421-1
5.	Туралин А.С. 421-1
6.	Деркачёв Е.И. 421-1
7.	Семёнов К.М. 421-1


### Обсуждалось:

1.	Проведённая работа за неделю:
  *	Фронт перешёл на React
  *	Доработали базу данных


### Дальнейшие действия:

2.	Фронтенд:
  *	Завершить переход на React
  *	Макеты для отчёта

3.	Бэкенд:
  *	Клиенты, лиды, сделки в базе. Взаимодействие с ними
  *	БД для отчёта

4.	Документация:
  *	Юз-кейсы

***

## 20.04.2023


### Присутствовали:

1.	Робонен А.С. 420-2
2.	Горяинов С.А. 420-3
3.	Васильчиков Д.А. 421-1
4.	Фёдоров Р.М. 421-1
5.	Туралин А.С. 421-1
6.	Семёнов К.М. 421-1


### Обсуждалось:

1.	Проведённая работа за неделю:
  *	Схематичные юз-кейсы
  *	(бэк) Рефакторинг
  *	Реализованы лиды


### Дальнейшие действия:

1.	Фронденд:
  *	Реализация входа

2.	Бэкэнд:
  *	Реализация компаний
  *	Реализация приглашений

3.	Документация:
  *	Доделать юз-кейсы
  *	(Доп) Переорганизовать вики

***

## 27.04.2023


### Присутствовали:

1.	Робонен А.С. 420-2
2.	Горяинов С.А. 420-3
3.	Васильчиков Д.А. 421-1
4.	Фёдоров Р.М. 421-1
5.	Туралин А.С. 421-1
6.	Деркачёв Е.И. 421-1
7.	Семёнов К.М. 421-1


### Обсуждалось:

1.	Майская работа:
  *	Презентация(фронт)
  *	Общий отчёт(аналитик)
  *	Скрины результатов (фронт, бэк)


### Дальнейшие действия:

2.	Фронтенд:
  *	Подготовка к презентации
  *	Скрины интерфейса

3.	Бэкэнд:
  *	Скрины БД, её функционала

4.	Документация:
  *	Подготовка к общему отчёту

***

## 04.05.2023


### Присутствовали:

1.	Робонен А.С. 420-2
2.	Горяинов С.А. 420-3
3.	Васильчиков Д.А. 421-1
4.	Фёдоров Р.М. 421-1
5.	Туралин А.С. 421-1
6.	Деркачёв Е.И. 421-1
7.	Семёнов К.М. 421-1


### Обсуждалось:

1.	Проведённая работа:
  *	Схема кодов приглашения
  *	Начата работа над макетами презентации и компонентами
2.	Итоговый отчёт:
  *	Части отчёта:
    *	Описание системы (проектирование, концепция, что-зачем-почему)
    *	Фронтэнд (что сделано, макеты, скрины)
    *	Бекэнд (что сделано, БД, проектирование БД)
  *	Аналитика:
    * Описание системы
    * Структура
    * Титульный лист
  *	Остальные:
    *	Описание выполненной работы 
    *	В начале документа расписать свои роли и задачи в общем виде в команде
  *	Формирование документа:
    * Подписывать рисунки
    * Упоминать рисунки в тексте
    *	Нельзя завершать разделы рисунками (после рисунка нужно ещё немного текста)
    *	Упоминать источники в [квадратные скобки] с ссылками и датами обращения
  *	Примечания:
    *	Писать в обезличенном формате (мы сделали --> было сделано)
    *	Дедлайн – конец мая
  *	Рассказ:
    *	Сначала объяснение общей концепции
    *	Затем индивидуально каждый отчитывается по своей роли, задачам, выполненной работе и изученных материалах
    *	Можно упомянуть о проблемах, с которыми столкнулись в процессе работы и как их решили
    *	Завершение



### Дальнейшие действия:

1.	Общее
  *	Отчёты о проделанной работе

2.	Фронтенд:
  *	Подготовка презентации

3.	Бэкэнд:
  *	Реализация инвайтов

4.	Документация:
  *	Расписать структуру страниц
  *	Подготовка материалов к отчёту
